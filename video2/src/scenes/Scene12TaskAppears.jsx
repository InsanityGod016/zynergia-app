...
/*
  ...
  f0–30    Roadmap shrinks + shifts further left; task card slides in from right
  f30–60   "Enviar mensaje" button fades in (WhatsApp green)
  f60–90   Hold

  LAYOUT: roadmap shrinks on left, task card on right
*/
...
// ── Roadmap node (animated shrink) ──

const RoadmapNode = ({label, isActive, yPos, shiftX, roadmapScale}) => {
  const baseNodeScale = isActive ? 1.5 : 0.8;
  const nodeScale = baseNodeScale * roadmapScale;
  ...
  return (
    <div
      style={{
        ...
        left: 540 + shiftX,
        top: yPos,
        transform: `translate(-50%, -50%) scale(${roadmapScale})`,
...
const ConnectingLine = ({fromIndex, isActiveSegment, shiftX, roadmapScale}) => {
  const fromY = START_Y + fromIndex * NODE_GAP + NODE_SIZE / 2;
  const lineHeight = (NODE_GAP - NODE_SIZE) * roadmapScale;
...
export const Scene12TaskAppears = () => {
  const frame = useCurrentFrame();
  const {fps} = useVideoConfig();

  // ── Roadmap shrinks and shifts further left ──
  const shrinkSpring = spring({
    frame,
    fps,
    config: {damping: 16, stiffness: 60, mass: 1.0},
  });
  const shiftX = interpolate(shrinkSpring, [0, 1], [-280, -400]);
  const roadmapScale = interpolate(shrinkSpring, [0, 1], [1, 0.7]);

  // ── Task card slides in from right ──
  ...
  const btnY = frame >= btnStart ? interpolate(btnSpring, [0, 1], [30, 0]) : 30;
  ...
  return (
    ...
    >
      ...
      {/* Task card — slides in from right, centered on screen */}
      <div
        style={{
          position: 'absolute',
          left: '50%',
          top: activeNodeY,
          transform: `translate(-30%, -50%) translateX(${cardX}px)`,
          opacity: cardOpacity,
